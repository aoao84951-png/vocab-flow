...
(() => {
  ...
  $('download').addEventListener('click', () => {
    ...
    const report = {version:9, experiment:'animated-caret-language-indicator', composer:'v8-no-keyup-latch', exported:new Date().toISOString(), userAgent:navigator.userAgent, platform:navigator.platform, maxTouchPoints:navigator.maxTouchPoints, standalone:matchMedia('(display-mode: standalone)').matches, trials};
    const url = URL.createObjectURL(new Blob([JSON.stringify(report,null,2)],{type:'application/json'}));
    const a = document.createElement('a'); a.href=url; a.download=`vocab-flow-focus-v9-${Date.now()}.json`; document.body.appendChild(a); a.click(); a.remove();

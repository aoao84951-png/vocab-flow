...
(() => {
  ...
  const composer = new window.LocalKeyboard(fields, () => !!active, entry => {
    ...
    if (entry.type === 'local-language-change') $('local-language').textContent = `${entry.language === 'ko' ? '한글' : 'English'} · Caps Lock으로 전환`;
  ...
  $('download').addEventListener('click', () => {
    ...
    const report = {version:7, experiment:'inputmode-none-local-caps-language', composer:'v5-with-physical-key-mapping', exported:new Date().toISOString(), userAgent:navigator.userAgent, platform:navigator.platform, maxTouchPoints:navigator.maxTouchPoints, standalone:matchMedia('(display-mode: standalone)').matches, trials};
    const url = URL.createObjectURL(new Blob([JSON.stringify(report,null,2)],{type:'application/json'}));
    const a = document.createElement('a'); a.href=url; a.download=`vocab-flow-focus-v7-${Date.now()}.json`; document.body.appendChild(a); a.click(); a.remove();

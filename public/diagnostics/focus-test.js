...
(() => {
  ...
  const trials = [];
  let active = null, timer = null, pending = false, transferred = false;
  ...
  const restore = () => { clearTimeout(timer); timer = null; pending = false; first.contentEditable = 'true'; };
  const end = () => {
    if (!active) return;
    log({ type: 'finish', state: snapshot() });
    active.final = snapshot(); active.ended = new Date().toISOString();
    active.check = active.final.first !== '입니다' ? '첫 칸 문구 확인 필요' : !active.firstInsert ? '둘째 칸 입력 없음' : active.firstInsert.data !== 'ㄹ' ? '첫 입력 불일치' : active.final.second !== '' ? '삭제 후 글자 남음' : '정상 조건 충족';
    ...
    active = null; restore();
    $('start').disabled = false; $('finish').disabled = true; $('mode').disabled = false; $('keyboard').disabled = false; $('outcome').disabled = false;
  };
  $('start').addEventListener('click', () => {
    restore(); transferred = false;
    first.textContent = ''; second.textContent = '';
    active = { mode: $('mode').value, keyboard: $('keyboard').value, started: new Date().toISOString(), clock: performance.now(), viewport: {width: innerWidth, height: innerHeight}, events: [], outcome: 'unknown', truncated: false };
    trials.push(active);
    $('start').disabled = true; $('finish').disabled = false; $('mode').disabled = true; $('keyboard').disabled = true; $('outcome').disabled = true; $('outcome').value = 'unknown';
  ...
  const move = (event, navigation) => {
    ...
    pending = true;
    if (active.mode === 'disable') first.contentEditable = 'false';
    first.blur();
    log({type:'transition-released', state:snapshot()});
    timer = setTimeout(() => {
      if (!active || !pending) return;
      second.focus();
      transferred = true; restore();
  ...
  // Prevent the synthetic click from bypassing the experimental wait.
  second.addEventListener('click', event => { if(pending) event.preventDefault(); });
  ...
  $('download').addEventListener('click', () => {
    ...
    const report = {version:1, experiment:'focus-transition-80ms', exported:new Date().toISOString(), userAgent:navigator.userAgent, platform:navigator.platform, maxTouchPoints:navigator.maxTouchPoints, standalone:matchMedia('(display-mode: standalone)').matches, trials};
    const url = URL.createObjectURL(new Blob([JSON.stringify(report,null,2)],{type:'application/json'}));
    const a = document.createElement('a'); a.href=url; a.download=`vocab-flow-focus-${Date.now()}.json`; document.body.appendChild(a); a.click(); a.remove();

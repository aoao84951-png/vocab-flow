"use client";
import { isBookFolder } from "@/lib/bookContents";
...
type Folder = { id: string; title: string; icon?: string; desc?: string; coverImage?: string; isBook?: boolean; folders: Folder[] };
export default function FolderInlineActions({folder,books,onAction,onAddDay,onClose}:{folder:Folder;books:Folder[];onAction:(action:FolderAction)=>void;onAddDay:()=>void;onClose:()=>void}) {
 ...
 const [isBook,setIsBook]=useState(isBookFolder(folder));
 ...
 return <div className="mb-2 rounded-xl border border-[#e7edf2] bg-[#f8fafc] p-2 text-[10px] text-[#687887]" aria-label={`${folder.title} 관리 영역`}>
  <div className="flex min-w-0 items-center gap-1">
   <div className="flex min-w-0 flex-1 flex-nowrap items-center gap-1 overflow-x-auto pb-0.5">
    ...
    {([{key:"add",label:"폴더 추가",Icon:FolderPlus},{key:"edit",label:"수정",Icon:Pencil},{key:"move",label:"이동",Icon:CornerUpRight},{key:"delete",label:"삭제",Icon:Trash2}] as const).map(({key,label,Icon})=><button key={key} aria-pressed={mode===key} onClick={()=>{setMode(key);if(key==="add"){setTitle("");setDesc("");setCoverImage("");setIsBook(false);setCoverBusy(false)}else if(key==="edit"){setTitle(folder.title);setDesc(folder.desc??"");setCoverImage(folder.coverImage??"");setIsBook(isBookFolder(folder));setCoverBusy(false)}}} className={`flex min-h-7 shrink-0 items-center gap-1 rounded-lg px-1 ${mode===key?"bg-[#e8f1f8] text-[#527895]":"hover:bg-white"}`}><Icon size={13} strokeWidth={1.6}/>{label}</button>)}
   </div>
   <button aria-label="폴더 관리 닫기" onClick={onClose} className="flex h-7 w-7 shrink-0 items-center justify-center"><X size={14}/></button>
  </div>
  {(mode==="edit"||mode==="add")&&<form className="mt-1.5 space-y-1.5" onSubmit={e=>{e.preventDefault();if(title.trim()&&!coverBusy)finish({kind:mode,id:folder.id,title:title.trim(),icon:mode==="edit"?(folder.icon??""):"",desc,coverImage,isBook})}}>
   <div className="flex gap-2"><input autoFocus aria-label={mode==="add"?"새 하위 폴더 이름":"폴더 이름"} placeholder="새 하위 폴더 이름" value={title} onChange={e=>setTitle(e.target.value)} className="min-w-0 flex-1 h-8 rounded-lg border border-[#e1e9ef] bg-white px-2 text-[12px] outline-none focus:border-[#9bbbd2]"/></div>
   <input aria-label="폴더 설명" placeholder="설명 (선택)" value={desc} onChange={e=>setDesc(e.target.value)} className="h-7 w-full rounded-lg border border-[#e1e9ef] bg-white px-2 text-[12px]"/>
   <BookCoverInput isBook={isBook} onBookChange={setIsBook} key={mode} value={coverImage} onChange={setCoverImage} onBusyChange={setCoverBusy}/>

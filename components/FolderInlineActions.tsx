...
export default function FolderInlineActions({folder,books,onAction,onClose}:{folder:Folder;books:Folder[];onAction:(action:FolderAction)=>void;onClose:()=>void}) {
 ...
 return <div className="mb-2 rounded-xl border border-[#e7edf2] bg-[#f8fafc] p-2 text-[11px] text-[#687887]" aria-label={`${folder.title} 관리 영역`}>
  <div className="flex min-w-0 items-center gap-1">
   <div className="flex min-w-0 flex-1 flex-nowrap items-center gap-1 overflow-x-auto pb-0.5">
    {([{key:"add",label:"추가",Icon:FolderPlus},{key:"edit",label:"수정",Icon:Pencil},{key:"move",label:"이동",Icon:CornerUpRight},{key:"delete",label:"삭제",Icon:Trash2}] as const).map(({key,label,Icon})=><button key={key} aria-pressed={mode===key} onClick={()=>{setMode(key);setSymbols(false);if(key==="add"){setTitle("");setIcon("#");setDesc("")}else if(key==="edit"){setTitle(folder.title);setIcon(folder.icon??"#");setDesc(folder.desc??"")}}} className={`flex min-h-8 shrink-0 items-center gap-1 rounded-lg px-1.5 ${mode===key?"bg-[#e8f1f8] text-[#527895]":"hover:bg-white"}`}><Icon size={13} strokeWidth={1.6}/>{label}</button>)}
   </div>
   <button aria-label="폴더 관리 닫기" onClick={onClose} className="flex h-8 w-7 shrink-0 items-center justify-center"><X size={14}/></button>
  </div>
  {(mode==="edit"||mode==="add")&&<form className="mt-2 space-y-2" onSubmit={e=>{e.preventDefault();if(title.trim())finish({kind:mode,id:folder.id,title:title.trim(),icon,desc})}}>
   <div className="flex gap-2"><button type="button" aria-label="폴더 기호 변경" aria-expanded={symbols} onClick={()=>setSymbols(!symbols)} className="folder-symbol h-9 w-9 shrink-0 rounded-lg bg-white text-base">{icon||"없음"}</button><input autoFocus aria-label={mode==="add"?"새 하위 폴더 이름":"폴더 이름"} placeholder="새 하위 폴더 이름" value={title} onChange={e=>setTitle(e.target.value)} className="min-w-0 flex-1 rounded-lg border border-[#e1e9ef] bg-white px-2 text-sm outline-none focus:border-[#9bbbd2]"/></div>
   {symbols&&<FolderSymbolPicker value={icon} onChange={setIcon}/>}
   <input aria-label="폴더 설명" placeholder="설명 (선택)" value={desc} onChange={e=>setDesc(e.target.value)} className="h-9 w-full rounded-lg border border-[#e1e9ef] bg-white px-2 text-sm"/>

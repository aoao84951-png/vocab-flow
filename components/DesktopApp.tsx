...
const applyBracketHighlightToHtml = (html: string) => {
  ...
  return html
    ...
    .map((part) => {
      ...
      return part.replace(
        /\[\[(.*?)\]\]/g,
        '<strong class="font-bold text-[#d92d20]">$1</strong>',
      );
    })
    .join("");
};
...
export default function DesktopApp() {
  ...
  const folderLongPressTimer = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const [wordSortOrder, setWordSortOrder] = useState<"latest" | "oldest">(
    "oldest",
  );
  const [wordViewMode, setWordViewMode] = useState<"all" | "unmemorized">(
    "all",
  );
  ...
  const getSwipeX = (index: number) => {
    if (draggingIndex === index)
      return Math.max(0, Math.min(dragX, SWIPE_WIDTH));
  ...
  async function saveBooks(next: Book[] | ((prev: Book[]) => Book[])) {
    const nextBooks = typeof next === "function" ? next(books) : next;
  ...
  const renderTopBar = (
    ...
    >
      <div className="flex w-[82px] items-center justify-start">
        <button
          onClick={onBack}
          className="flex h-9 w-9 items-center justify-center pt-[2px] text-[#75639b]"
          aria-label="뒤로가기"
        >
          <ChevronLeft />
        </button>
  ...
  }, [step, wordIndex, visibleSortedWords]);
  ...
  const deleteDay = (targetDayId: string) => {
    ...
    saveBooks((prev) => deleteDayFromPath(prev, folderPath, targetDayId));
  ...
  const addFolderToPath = (
    ...
    return folders.map((folder) =>
      folder.id === path[0]
        ? {
            ...folder,
            folders: addFolderToPath(folder.folders, path.slice(1), newFolder),
          }
        : folder,
    );
  };
    ...
    return folders.map((folder) =>
      folder.id === path[0]
        ? path.length === 1
          ...
          : {
              ...folder,
              folders: addDayToPath(folder.folders, path.slice(1), newDay),
            }
        : folder,
    );
  };
  ...
  const deleteFolderFromPath = (
    ...
    return folders
      ...
      .map((folder) => ({
        ...folder,
        folders: deleteFolderFromPath(folder.folders, targetFolderId),
      }));
  };
  ...
  const findFolderPathById = (
    ...
    for (const folder of folders) {
      ...
      const found = findFolderPathById(
        folder.folders,
        targetFolderId,
        nextPath,
      );
      ...
      folder.id === path[0]
        ? path.length === 1
          ? {
              ...folder,
              days: (() => {
                const index = folder.days.findIndex(
                  (day) => day.id === targetDayId,
                );
                if (index === -1) return folder.days;

                const nextIndex = direction === "up" ? index - 1 : index + 1;
                if (nextIndex < 0 || nextIndex >= folder.days.length)
                  return folder.days;
  ...
  return (
    <main className="min-h-[100svh] overscroll-none bg-white text-[#111827]">
      ...
      <section className="min-h-[100svh] w-full bg-white px-3 sm:px-5 md:px-6 lg:px-8">
        ...
        {step !== "book" && selectedBook && activeFolder && (
          <button
            onClick={() => setMenuOpen(true)}
            className="fixed bottom-[calc(116px+env(safe-area-inset-bottom))] right-4 z-50 flex h-10 w-10 items-center justify-center rounded-full border border-[#e4e8f0] bg-white/90 shadow-[0_8px_24px_rgba(15,23,42,0.10)] backdrop-blur transition active:scale-95"
            aria-label="목록 열기"
          >
          ...
          <div className="min-h-dvh bg-white px-5 pt-8 pb-6">
            <div>
              <div className="flex items-center justify-between">
                <h1 className="text-[28px] font-bold tracking-tight text-[#75639b]">
                  단어장
                </h1>
                ...
                <button
                  ...
                  className="h-[38px] rounded-full bg-[#e3daf5] px-5 text-[12px] font-bold text-[#51426c]"
                >
                  + 추가
                </button>
                </div>
                    ...
                    <div key={book.id} className="relative">
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <span className="ipad-list-text folder-title truncate text-[16px] font-medium tracking-[-0.03em] text-[#303236]">
                            {book.title}
                          </span>
                        </div>

                        <span className="ml-3 flex h-5 w-5 shrink-0 items-center justify-center text-[#9aa3b2]">
            ...
            <div className="mt-6 flex w-full items-center justify-between px-8 sm:px-10 md:px-11 lg:px-[52px]">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="rounded-full bg-[#f3eff9] px-4 py-2 text-[12px] font-bold text-[#75639b]"
                >
                  + Folder
                </button>
                ...
                <button
                  ...
                  className="rounded-full bg-[#e3daf5] px-4 py-2 text-[12px] font-bold text-[#51426c]"
                >
                  + Day
                </button>
              </div>
                  ...
                  <div className="flex min-w-0 flex-1 items-center justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex min-w-0 items-center gap-2">
                        <p className="ipad-list-text truncate text-[15px] font-semibold tracking-[-0.03em] text-[#303236]">
                          {day.title}
                        </p>
                        ...
                        <span
                          ...
                          style={{
                            fontFamily:
                              '"Summer Symbols", -apple-system, BlinkMacSystemFont, "Apple SD Gothic Neo", "Segoe UI", sans-serif',
                          }}
                        >
                          {getDayProgress(day)}
                        </span>
            ...
            <div className="mx-8 mt-4 flex rounded-full bg-[#f5f6fa] p-1 sm:mx-10 md:mx-11 lg:mx-[52px]">
              <button
                ...
                className={`h-8 flex-1 rounded-full text-[12px] font-bold ${
                  wordViewMode === "all"
                    ? "bg-white text-[#75639b] shadow-[0_2px_8px_rgba(15,23,42,0.06)]"
                    : "text-[#8a94a6]"
                }`}
              >
                전체보기
                  ...
                  >
                    ...
                    <button
                      ...
                      className={`relative z-10 w-full rounded-[18px] border px-4 py-4 text-left shadow-[0_3px_10px_rgba(15,23,42,0.04)] transition-transform active:scale-[0.99] ${
                        item.importanceStars
                          ? "border-[#b9c9ed] bg-[#f8fbff] shadow-[0_5px_16px_rgba(117,99,155,0.10)]"
                        ...
                        <div className="min-w-0">
                          {item.importanceStars ? (
                            <span className="mb-1 inline-flex items-center">
                              <ImportanceStars
                                count={item.importanceStars}
                                size={13}
                              />
                            </span>
                          ) : null}
                          <p
                            className={`ipad-word-title truncate text-[17px] font-bold ${
                              ...
                              {(item.meanings ?? []).length > 0 ? (
                                (item.meanings ?? []).map(
                                  (group, groupIndex) => (
                                    ...
                                    >
                                      <span className="ipad-pos-badge inline-flex h-[16px] min-w-[16px] items-center justify-center rounded-[4px] bg-[#e3daf5] text-[10px] font-bold text-[#51426c]">
        ...
        {step === "study" && selectedBook && selectedDay && (
          ...
          >
            ...
            <header
              onClick={(e) => e.stopPropagation()}
              className="sticky top-0 z-20 flex h-[76px] shrink-0 items-center justify-between border-b border-[#d7ddea] bg-[#f8fafc]/95 px-5 shadow-[0_1px_0_rgba(117,99,155,0.03)] backdrop-blur"
            >
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  className="flex h-9 w-9 items-center justify-center rounded-full text-[#75639b] active:scale-95"
                  aria-label="단어 메뉴"
                >
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                    <circle
            ...
            {!currentWord ? (
              ...
              <>
                  ...
                  <div className="absolute right-5 top-5 flex items-center gap-2">
                    ...
                    >
                      <span className="relative flex h-5 w-5 items-center justify-center">
                        <StarIcon
                          active={Boolean(currentWord.importanceStars)}
                          size={22}
                        />
                        {currentWord.importanceStars ? (
                          <span className="absolute -right-1 -top-1 flex h-[14px] min-w-[14px] items-center justify-center rounded-full bg-[#e3daf5] px-[3px] text-[8px] font-black leading-none text-[#51426c]">
                            {currentWord.importanceStars}
                          </span>
                        ) : null}
                      </span>
                    ...
                    <button
                      ...
                      className={`flex h-8 w-8 items-center justify-center rounded-full border transition-all ${
                        currentWord.memorized
                          ? "border-[#75639b] bg-[#e3daf5] shadow-[0_6px_14px_rgba(117,99,155,0.22)]"
                          : "border-[#e5deef] bg-[#f8fafc]"
                      }`}
                      aria-label="암기완료"
                    >
                      <svg
                      ...
                      >
                        <path
                          ...
                          stroke={currentWord.memorized ? "#75639b" : "#9aa3b2"}
                          strokeWidth="3"
                          strokeLinecap="round"
                          strokeLinejoin="round"
                        />
                          ...
                          return shouldCenterStackedNumberedMeanings ? (
                            ...
                            >
                              <div className="grid w-fit max-w-full grid-cols-[auto_minmax(0,1fr)] items-start gap-2">
                                <span className="ipad-detail-pos inline-flex h-[25px] min-w-[25px] items-center justify-center rounded-[7px] bg-[#e3daf5] px-[6px] text-[13px] font-bold text-[#51426c]">
                                  {group.pos}
                                </span>
                            ...
                            >
                              <span className="ipad-detail-pos mr-2 inline-flex h-[25px] min-w-[25px] items-center justify-center rounded-[7px] bg-[#e3daf5] px-[6px] text-[13px] font-bold text-[#51426c] align-top">
                                {group.pos}
                              </span>
                        ...
                        <div className="divide-y divide-[#d7ddea]">
                          {(currentWord.studyPoints ?? []).map(
                            (point, index) => (
                              ...
                              >
                                <div className="flex items-center gap-2">
                                  <span className="rounded-full bg-[#e7ecf5] px-2 py-1 text-[11px] font-bold text-[#75639b]">
                                    {point.category}
                                  </span>
                                          ...
                                          {variant.related && (
                                            <div className="mt-2 flex items-baseline gap-1.5 pl-[1px]">
                                              ...
                                              <div className="flex min-w-0 flex-1 items-start gap-2">
                                                <PronounceButtons text={variant.related} className="ipad-study-sub relative -top-[1.5px] min-w-0 flex-1 text-[12px] font-bold leading-[1.5] tracking-[-0.01em] text-[#4b6cb7]">
                                                  <HighlightedText
                                ...
                                {(() => {
                                  const pointExamples = point.examples?.length
                                    ? point.examples
                ...
                <footer
                  onClick={(e) => e.stopPropagation()}
                  className="fixed inset-x-0 bottom-0 z-20 border-t border-[#d7ddea] bg-[#f8fafc]/95 px-6 py-4 pb-[calc(16px+env(safe-area-inset-bottom))] shadow-[0_-1px_0_rgba(117,99,155,0.03)] backdrop-blur"
                >
                  <div className="mx-auto flex max-w-[820px] items-center justify-between gap-4">
                    <button
                      onClick={prevWord}
                      className="flex h-12 min-w-[150px] items-center justify-center gap-2 rounded-[14px] border border-[#d7ddea] bg-white text-[15px] font-bold text-[#75639b] shadow-[0_4px_12px_rgba(117,99,155,0.08)] active:scale-[0.98]"
                    >
                      ← 이전
                    </button>
                    ...
                    <button
                      onClick={nextWord}
                      className="flex h-12 min-w-[150px] items-center justify-center gap-2 rounded-[14px] bg-[#e3daf5] text-[15px] font-bold text-[#51426c] shadow-[0_5px_14px_rgba(117,99,155,0.16)] active:scale-[0.98]"
                    >
                      다음 →
                    </button>
                  </div>
        ...
        {step === "addWord" && activeFolder && (
          <AddWord
            ...
            onSave={(dayId, word) => {
              saveBooks((prev) => addWordToPath(prev, folderPath, dayId, word));
        ...
        {step === "moveFolder" && actionFolderId && (
          <div className="min-h-dvh px-5 pt-7 pb-6">
            ...
            <h1 className="mt-4 text-[28px] font-bold text-[#75639b]">
              폴더 이동
            </h1>
            ...
            <div className="mt-5 space-y-1">
              <button
                ...
                onClick={() => {
                  saveBooks((prev) =>
                    moveFolderToFolder(prev, actionFolderId, null),
                  );
                ...
                className={`w-full rounded-2xl border border-[#e5deef] bg-[#f8fafc] px-4 py-4 text-left text-[14px] font-bold text-[#75639b] ${
          ...
          >
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <h2 className="pt-1 text-[16px] font-extrabold tracking-[-0.05em] text-[#75639b]">
                  목차
                </h2>
                ...
                <button
                  ...
                  className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#f4f6fb] text-[#8b95a5] transition active:scale-95"
                  aria-label="목차 닫기"
                >
                  <svg
                    width="16"
              ...
              <div className="relative mt-4">
                <button
                  ...
                  className={`flex h-12 w-full items-center gap-3 rounded-[20px] border bg-[#fbfcff] px-3.5 text-left transition active:scale-[0.99] ${
                    bookDropdownOpen
                      ? "border-[#0965d8] shadow-[0_0_0_2px_rgba(9,101,216,0.04)]"
                      : "border-[#e0e5ef]"
                  }`}
                >
                  <span className="min-w-0 flex-1 truncate text-[14px] font-extrabold tracking-[-0.04em] text-[#75639b]">
                    {selectedBook.title}
                  </span>
                ...
                {bookDropdownOpen && (
                  <div className="mt-2 rounded-[20px] border border-[#e0e5ef] bg-white p-2 shadow-[0_14px_34px_rgba(15,23,42,0.12)]">
                          ...
                          >
                            ...
                            {isSelected && (
                              <svg
                                ...
                                viewBox="0 0 24 24"
                                fill="none"
                                className="shrink-0 text-[12px] text-[#75639b]"
                                aria-hidden="true"
                              >
                                <path
                                  d="M5 12.4L9.2 16.6L19 7"
                                  stroke="currentColor"
                                  strokeWidth="3"
                                  strokeLinecap="round"
                                  strokeLinejoin="round"
        ...
        {actionWordIndex !== null && selectedDay && (
          ...
          >
            ...
            >
              <div className="overflow-hidden rounded-[14px] bg-white/95 shadow-[0_18px_45px_rgba(117,99,155,0.18)] backdrop-blur-xl">
                ...
                <button
                  ...
                  className="h-[54px] w-full border-b border-[#e5e8ef] text-center text-[18px] font-normal text-[#007aff] active:bg-[#f3eff9]"
                >
                  수정
                </button>
              ...
              <button
                onClick={() => setActionWordIndex(null)}
                className="h-[54px] w-full rounded-[14px] bg-white/95 text-center text-[18px] font-semibold text-[#007aff] shadow-[0_12px_28px_rgba(117,99,155,0.12)] active:bg-[#f3eff9]"
              >
                취소
              </button>
            </div>
              ...
              >
                <div className="flex select-none items-center justify-between">
                  <p className="translate-x-[8px] text-[16px] font-bold tracking-[-0.03em] text-[#111827]">
                    {
                      activeFolder?.days.find((day) => day.id === actionDayId)
                        ?.title
                    }
                  </p>
                  ...
                  <div className="relative h-9 w-9">
                    ...
                    <div
                      className="pointer-events-none flex h-5 w-5 translate-y-[3px] items-center justify-center text-[13px] font-bold leading-none text-[#8a94a6]"
                      style={{
                        fontFamily:
                          '"Summer Symbols", "Summer Melody", sans-serif',
                      }}
                    >
                      ↕
                    </div>
                ...
                <div className="mt-4 space-y-2">
                  <button
                    ...
                    className="h-12 w-full rounded-2xl bg-[#f3eff9] text-[13px] font-bold text-[#75639b]"
                  >
                    Day 수정
                  </button>
                  ...
                  <button
                    onClick={() => setActionDayId(null)}
                    className="h-12 w-full rounded-2xl border border-[#e5deef] text-[13px] text-[#596275]"
                  >
                    취소
                  </button>
                </div>
                  ...
                  <div className="relative h-9 w-9">
                    <select
                      ...
                      onChange={(e) => {
                        if (e.target.value === "up") {
                          saveBooks((prev) =>
                            moveFolderOrder(prev, actionFolderId, "up"),
                          );
                        }

                        if (e.target.value === "down") {
                          saveBooks((prev) =>
                            moveFolderOrder(prev, actionFolderId, "down"),
                          );
                        }
                ...
                <div className="mt-4 space-y-2">
                  ...
                  <button
                    onClick={() => {
                      ...
                      saveBooks((prev) =>
                        deleteFolderFromPath(prev, actionFolderId),
                      );
                  ...
                  <button
                    onClick={() => setActionFolderId(null)}
                    className="h-12 w-full rounded-2xl border border-[#e5deef]"
                  >
                    취소
                  </button>
                </div>
    ...
    <div className="min-h-dvh px-5 pt-7 pb-6">
      ...
      <div className="mt-7 space-y-4">
        ...
        <button
          ...
          className="h-12 w-full rounded-full bg-[#e3daf5] text-[13px] font-bold text-[#51426c]"
        >
          저장
        </button>
...
function AddFolder({
  ...
  return (
    <div className="min-h-dvh px-5 pt-7 pb-6">
      ...
      <h1 className="mt-4 text-[28px] font-bold text-[#75639b]">{titleText}</h1>
...
function AddWord({
  ...
  const updateMeaningGroup = (groupIndex: number, next: Partial<Meaning>) => {
    setMeanings((prev) =>
      prev.map((group, i) =>
        i === groupIndex ? { ...group, ...next } : group,
      ),
    );
  };

  const updateMeaningItem = (
    groupIndex: number,
    itemIndex: number,
    value: string,
  ) => {
  ...
  const updateStudyPoint = (
    index: number,
    key: keyof StudyPoint,
    value: string,
  ) => {
    setStudyPoints((prev) =>
      prev.map((point, i) =>
  ...
  const updateStudyPointVariantWord = (
    pointIndex: number,
    variantIndex: number,
    value: string,
  ) => {
  ...
  const updateStudyPointVariantRelated = (
    pointIndex: number,
    variantIndex: number,
    value: string,
  ) => {
  ...
  const addStudyPointVariantMeaning = (
    pointIndex: number,
    variantIndex: number,
  ) => {
    setStudyPoints((prev) =>
                          ...
                          ? {
                              ...
                              items:
                                meaning.items.length > 1
                                  ? meaning.items.filter(
                                      (_, l) => l !== itemIndex,
                                    )
                                  : [""],
                              numbered:
                                meaning.items.length > 2
                                  ? meaning.numbered
                                  : false,
                            }
                          : meaning,
    ...
    setStudyPoints((prev) =>
      prev.map((point, i) =>
        i === pointIndex
          ? {
              ...point,
              variants: (point.variants ?? []).filter(
                (_, j) => j !== variantIndex,
              ),
            }
          : point,
  ...
  const meaningOptions = meanings.flatMap((group, groupIndex) =>
    group.items.map((item, itemIndex) => ({
      ...
      label: group.numbered ? `${group.pos} ${itemIndex + 1}` : group.pos,
      text: item,
    })),
  );
  ...
  return (
    <div className="min-h-dvh px-5 pt-7 pb-6">
      {/* 아이패드에서 Tab으로 다음 칸으로 넘어갈 때, 그 사이에 잠깐
          ...
          한 번은 readOnly로 안 두면(= "진짜 편집 가능한 칸"으로 만들면)
          아래쪽 입력 소스 표시가 깜빡이는 건 없어지지만, 그 대신 이
          칸이 진짜 입력 대상 취급을 받으면서 이전 칸의 새는 한글 버퍼를
          그대로 받아버리고, 그게 다시 다음 칸으로 넘어가면서 원래
          고쳤던 "글자가 새어 들어오는" 버그가 되살아나는 걸 실제
          테스트로 확인했다. 즉 "표시 안 깜빡이게 하기"와 "값이 새지
          않게 하기"가 이 칸의 readOnly 여부를 놓고 서로 충돌한다.

          데이터가 깨지는 것(새는 값)이 화면이 잠깐 깜빡이는 것보다
          훨씬 심각한 문제이므로, readOnly를 유지해서 "값이 새지 않는
          것"을 우선한다. 이 경우 아래 입력 소스 표시가 Tab을 누를
          때마다 잠깐 깜빡이는 건 현재 구조에서 감수해야 하는
          부작용이다. */}
      <input
        ...
        <label className="block">
          ...
          <div className="relative">
            <select
              ...
              className="h-12 w-full appearance-none rounded-2xl border border-[#e5deef] px-4 pr-11 text-[14px] outline-none"
            >
              {book.days.map((day) => (
        ...
        <div>
          <div className="mb-2 flex items-center justify-between">
            ...
            <button
              ...
              className="rounded-full bg-[#f3eff9] px-3 py-1.5 text-[11px] font-bold text-[#75639b]"
            >
              + 품사 추가
            </button>
          </div>
          ...
          <div className="space-y-3">
            {meanings.map((group, groupIndex) => (
              <div
                key={groupIndex}
                className="rounded-2xl border border-[#e5deef] p-4"
              >
                <div className="flex gap-2">
                  <div className="relative w-[66px] shrink-0">
                    <select
                      ...
                      className="h-10 w-full appearance-none rounded-xl border border-[#e5deef] pl-5 pr-7 text-[13px] outline-none"
                    >
                  ...
                  <button
                    ...
                    className={`h-10 flex-1 rounded-xl border text-[12px] font-bold ${
                      group.numbered
                        ? "border-[#75639b] bg-[#f3eff9] text-[#75639b]"
                        : "border-[#e5deef] text-[#8a94a6]"
                    }`}
                  >
                    <span className="tracking-[0.08em]">① ②</span>
                    <span className="ml-1">
                      표시 {group.numbered ? "ON" : "OFF"}
                    </span>
                  </button>

                  <button
                    onClick={() => removeMeaningGroup(groupIndex)}
                    className="h-10 w-10 rounded-xl border border-[#e5deef] text-[13px] text-[#8a94a6]"
                  >
                    ×
                  </button>
                </div>
                  ...
                  {group.items.map((item, itemIndex) => (
                    <div key={itemIndex} className="relative">
                      <input
                        ...
                        placeholder={
                          group.numbered ? `${itemIndex + 1}번째 뜻` : "뜻 입력"
                        }
                        className="h-11 w-full rounded-xl border border-[#e5deef] px-3 pr-10 text-[13px] outline-none"
                      />
            ...
            {examples.map((example, index) => (
              ...
              >
                ...
                <input
                  ...
                  placeholder="영어 예문"
                  className="h-11 w-full rounded-xl border border-[#e5deef] px-3 text-[13px] outline-none"
                />
                ...
                <input
                  ...
                  placeholder="한국어 해석"
                  className="mt-2 h-11 w-full rounded-xl border border-[#e5deef] px-3 text-[13px] outline-none"
                />
              </div>
            ))}
          </div>
              ...
              >
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <select
                      ...
                      className="h-10 w-full appearance-none rounded-xl border border-[#e5deef] px-3 pr-9 text-[13px] outline-none"
                    >
                ...
                {!STUDY_CATEGORIES.includes(point.category) && (
                  <input
                    ...
                    placeholder="유형 직접입력"
                    className="mt-3 h-10 w-full rounded-xl border border-[#e5deef] px-3 text-[13px] outline-none"
                  />
                )}
                ...
                <input
                  ...
                  placeholder="제목"
                  className="mt-3 h-11 w-full rounded-xl border border-[#e5deef] px-3 text-[13px] outline-none"
                />
                ...
                <div className="mt-3 rounded-2xl bg-[#f8fafc] p-3">
                  ...
                  <div className="space-y-2">
                    {(point.variants ?? []).map((variant, variantIndex) => (
                      <div
                        key={variantIndex}
                        className="rounded-xl border border-[#e5deef] bg-white p-3"
                      >
                        <div className="flex gap-2">
                          <input
                            ...
                            className="h-10 min-w-0 flex-1 rounded-xl border border-[#e5deef] px-3 text-[13px] outline-none"
                          />
                            ...
                            >
                              <div className="flex gap-2">
                                <div className="relative w-[66px] shrink-0">
                                  <select
                                    value={meaning.pos}
                                    onChange={(e) =>
                                      updateStudyPointVariantMeaning(
                                        index,
                                        variantIndex,
                                        meaningIndex,
                                        {
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    removeStudyPointVariantMeaning(
                                      index,
                                      variantIndex,
                                      meaningIndex,
                                    )
                              ...
                              <div className="mt-2 flex justify-end">
                                <button
                                  type="button"
                                  onClick={() =>
                                    addStudyPointVariantMeaningItem(
                                      index,
                                      variantIndex,
                                      meaningIndex,
                                    )
                                  }
                                  className="rounded-full bg-white px-3 py-1.5 text-[11px] font-bold text-[#75639b]"
                                >
                                  + 뜻 추가
                                </button>
                              </div>
                      ...
                      >
                        <div className="mb-2 flex items-center justify-between">
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              removeStudyPointExample(index, exampleIndex)
                            }
                            className="flex h-5 w-5 items-center justify-center rounded-full bg-[#f5f6fa] text-[15px] text-[#8a94a6]"
                            aria-label="예시 삭제"
                          >
            ...
            const cleanedStudyPoints = studyPoints
              .map((point, index) => {
                ...
                const description = cleanEditorHtml(
                  studyDescriptionRefs.current[index]?.innerHTML ??
                    point.description ??
                    "",
                );
                const related = "";
                const examples = (
                  ...
                  .map((example, exampleIndex) => ({
                    en: cleanEditorHtml(
                      studyExampleEnRefs.current[`${index}-${exampleIndex}`]
                        ?.innerHTML ??
                        example.en ??
                        "",
                    ),
                ...
                const variants = (point.variants ?? [])
                  .map((variant) => {
                    const meanings = (variant.meanings ?? [])
                      ...
                      .filter(
                        (meaning) => meaning.pos && meaning.items.length > 0,
                      );
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        placeholder={placeholder}
        className="h-12 w-full rounded-2xl border border-[#e5deef] px-4 text-[14px] outline-none"
      />
    </label>
  );
}
  ...
  return (
    <label className="block">
      ...
      <textarea
        ...
        placeholder={placeholder}
        rows={rows}
        className="max-h-[92px] w-full resize-none overflow-y-auto rounded-2xl border border-[#e5deef] px-4 py-3 text-[14px] outline-none"
      />
    </label>
  );
}
    ...
    <div>
      ...
      <div className="space-y-3">
        {safeItems.map((item, index) => (
          <div key={index} className="rounded-2xl border border-[#e5deef] p-4">
            <div className="flex gap-2">
              <div className="relative w-[100px] shrink-0">
                <select
                  ...
                  onChange={(e) => {
                    const value = e.target.value;
                    const option = meaningOptions.find(
                      (opt) => opt.value === value,
                    );
                  ...
                  className="h-11 w-full appearance-none rounded-xl border border-[#e5deef] px-3 pr-8 text-[12px] outline-none"
                >
              ...
              <div className="relative min-w-0 flex-1">
                <input
                  ...
                  placeholder={placeholder}
                  className="h-11 w-full rounded-xl border border-[#e5deef] px-3 pr-12 text-[13px] outline-none"
                />
...
function Empty({ text }: { text: string }) {
  return (
    <div className="mt-8 rounded-[16px] border border-dashed border-[#e5deef] p-8 text-center text-[13px] text-[#8a94a6]">
      {text}
    </div>
  );
}
    ...
    <div className="border-b border-[#d7ddea] py-5 last:border-b-0">
      {title ? (
        <div className="grid grid-cols-[88px_minmax(0,1fr)] items-start gap-x-6">
          <p
            className={`ipad-block-title pt-[1px] text-[14px] font-extrabold tracking-[-0.01em] text-[#7d8ca3] ${
              title === "반의어" ? "pl-[1px]" : ""
            }`}
          >
            {title}
...
function ChipList({
  ...
  const wordChipClass =
    tone === "red"
      ...
      : "rounded-full bg-[#f3eff9] px-3 py-1 text-[12px] leading-none ipad-chip text-[#75639b]";
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        className={`w-full rounded-2xl border border-[#e5deef] px-4 py-4 text-left ${
          disabled ? "opacity-35" : ""
        }`}
      >
        <FolderSymbol symbol={folder.icon} />{folder.title}
...
function EditorBox({
  ...
  return (
    <div
      ...
      className="min-h-[96px] w-full whitespace-pre-wrap rounded-b-xl border border-t-0 border-[#e5deef] bg-white px-3 py-3 text-[13px] leading-[1.8] text-[#303236] outline-none empty:before:text-[#a3abb8] empty:before:content-[attr(data-placeholder)]"
    />
  );
}
...
function InlineEditorBox({
  ...
  return (
    <div
      ...
      data-placeholder={placeholder}
      onInput={(e) => resetEditorIfEmpty(e.currentTarget)}
      className="min-h-[44px] w-full whitespace-pre-wrap rounded-b-xl border border-t-0 border-[#e5deef] bg-white px-3 py-3 text-[13px] leading-[1.5] text-[#303236] outline-none empty:before:text-[#a3abb8] empty:before:content-[attr(data-placeholder)]"
    />
  );
}
...
function EditorToolbar({
  ...
  return (
    <div className="relative flex min-h-8 flex-wrap items-center gap-1 rounded-t-xl border border-[#e5deef] bg-[#f8fafc] px-2 py-1.5">
...
function ToolIcon({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className="flex h-6 min-w-6 items-center justify-center rounded-[5px] border border-[#cfd6e3] bg-white px-1.5 text-[11px] font-bold text-[#303236] active:bg-[#f3eff9]"
    >
      {children}
    </button>
  );
      ...
      <div className="absolute left-0 top-8 z-50 w-[220px] rounded-[12px] border border-[#cfd6e3] bg-white p-2 shadow-[0_10px_30px_rgba(15,23,42,0.18)]">
        <div className="flex flex-wrap items-center gap-1.5">
          <button
            ...
            className="h-6 rounded-md border border-[#e5deef] px-2 text-[10px] font-bold text-[#596275]"
          >
            없음
          </button>
        ...
        <div className="mt-3 flex items-center gap-1.5">
          <input
            ...
            className="h-7 w-8 rounded-md border border-[#e5deef] bg-white"
          />
          ...
          <input
            ...
            placeholder="#000000"
            maxLength={7}
            className="h-7 min-w-0 flex-1 rounded-md border border-[#e5deef] px-2 text-[11px] font-bold text-[#596275] outline-none"
          />

          <button
            type="button"
            onClick={() => onAdd(newColor)}
            className="h-7 rounded-md bg-[#e3daf5] px-2 text-[10px] font-bold text-[#51426c]"
          >
            추가
          </button>
        </div>
      ...
      {folders.map((folder, index) => {
        ...
        return (
          <div
            key={folder.id}
            className={
              depth === 0 ? "overflow-hidden rounded-[16px] bg-white" : ""
            }
          >
            <div
            ...
            >
              ...
              >
                <span
                  className={`truncate tracking-[-0.03em] ${
                    depth === 0
                      ? "ipad-list-text text-[15px] font-semibold text-[#303236]"
              ...
              {hasChildren ? (
                <button
                  ...
                  className="ml-2 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#e3daf5] text-[#51426c] active:scale-95"
                >
...
function MenuContentFolder({
  ...
  onSelectDay: (path: string[], dayId: string) => void;
}) {
  const hasChildren = folder.folders.length > 0 || folder.days.length > 0;
    ...
    <div>
      ...
      >
        ...
        <button
          ...
          className={`min-w-0 flex-1 truncate text-left text-[13px] tracking-[-0.04em] ${isSelected ? "font-extrabold text-[#75639b]" : "font-semibold text-[#47505f]"}`}
        >
          <FolderSymbol symbol={folder.icon} />{folder.title}
        </button>
      </div>
        ...
        <div className="mt-1 space-y-1">
          ...
          {folder.days.map((day) => (
            <MenuContentDay
              ...
              depth={depth + 1}
              selected={day.id === selectedDayId}
              onSelect={() => onSelectDay(path, day.id)}
            />
          ))}
        </div>
      )}
...
function MenuContentDay({
  ...
  return (
    ...
    >
      ...
      <span
        className={`min-w-0 flex-1 truncate text-[13px] tracking-[-0.04em] ${selected ? "font-extrabold text-[#75639b]" : "font-semibold text-[#47505f]"}`}
      >
        {day.title}
      </span>
    </button>
    ...
    <div>
      <div
        className={`flex w-full items-center rounded-xl px-2.5 py-2 ${
          isSelected ? "bg-[#f8fbff] text-[#75639b]" : "text-[#111827]"
        }`}

...
export default function ContentsAddForm({ kind, root, parentTitle, onCancel, onSubmit }: Props) {
  ...
  return <form aria-label={`${label} 추가`} className="mb-3 space-y-2 rounded-xl border border-[#e7edf2] bg-[#f8fafc] p-3" onSubmit={event => { event.preventDefault(); if (title.trim() && !busy) onSubmit({ title: title.trim(), desc: desc.trim(), coverImage, isBook }); }}>

...
export default function BottomNavigation({ books, step, path, dayId, onHome, onNavigate, onAdd, onFolderAction }: Props) {
  ...
  const tree = (items: Folder[], parent: string[] = []) => items.map(folder => {
    ...
    </div>{managing === folder.id && <FolderInlineActions folder={folder} books={books} onAction={onFolderAction} onClose={() => setManaging(null)}/>} {isOpen && <div className="ml-3 border-l border-[#e7edf2] pl-[19px]">{tree(folder.folders, next)}{folder.days.map(day => <button key={day.id} onClick={() => navigate(next, day.id)} className={`block min-h-11 w-full rounded-lg pl-2 pr-2 text-left text-sm text-[#4b5058] hover:bg-[#f7fafc] ${day.id === dayId ? "bg-[#eff7fc]" : ""}`}>{day.title}<span className="ml-2 text-xs text-[#939ba5]">{day.words.length}개</span></button>)}</div>}</div>;
  });
    ...
    {panel && <div className="fixed inset-0 z-[70] bg-black/15" onClick={() => setPanel(null)}>
      <section role="dialog" aria-modal="true" aria-label={{search:"전체 검색",stars:"중요 단어",add:"추가",contents:"목차"}[panel]} onClick={e => e.stopPropagation()} className="absolute inset-x-3 bottom-[calc(98px+var(--mobile-bottom-inset))] mx-auto flex max-h-[calc(100dvh-140px-var(--mobile-bottom-inset)-env(safe-area-inset-top))] max-w-[640px] flex-col rounded-3xl bg-white px-5 pt-5 pb-4 shadow-xl sm:inset-x-0 sm:bottom-[108px] sm:max-h-[72dvh]">
        ...
        <div ref={panelScrollRef} onScroll={showPanelScrollbar} data-bottom-panel-scroll className="min-h-0 overflow-y-auto overscroll-contain">
          ...
          {panel === "contents" && <p className="mb-2 text-xs text-[#858b94]">목차를 꾹 눌러 이동 · 가운데에 놓으면 하위 목차로 이동</p>}

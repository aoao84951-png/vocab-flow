...
export default function BottomNavigation({ books, step, path, dayId, onHome, onNavigate, onAdd, onFolderAction }: Props) {
  ...
  const tree = (items: Folder[], parent: string[] = []) => items.map(folder => {
    ...
    const drop = drag?.drop?.id === folder.id ? drag.drop.position : null;
    return <div key={folder.id} className={drag?.id === folder.id ? "opacity-40" : ""}><div data-folder-row={folder.id} data-folder-title={folder.title} data-folder-path={JSON.stringify(next)} className={`relative flex min-h-11 items-center gap-0 ${drop === "inside" ? "rounded-lg bg-[#dceefa] ring-2 ring-[#87b5d4]" : ""}`}>
      {drop && drop !== "inside" && <div className={`pointer-events-none absolute inset-x-0 h-0.5 bg-[#629bc3] ${drop === "before" ? "top-0" : "bottom-0"}`} />}
      <button data-folder-grab title="길게 눌러 이동" style={{ WebkitTouchCallout: "none", userSelect: "none", cursor: drag ? "grabbing" : "grab" }} aria-expanded={hasChildren ? isOpen : undefined} className="flex min-w-0 flex-1 items-center py-2 text-left text-sm text-[#4b5058]" onClick={toggle}><FolderSymbol symbol={folder.icon} /><span className="min-w-0 truncate">{folder.title}</span></button>
      <button aria-label={`${folder.title} 관리`} title="Day·폴더 추가 및 관리" aria-expanded={managing === folder.id} onClick={() => setManaging(managing === folder.id ? null : folder.id)} className="flex h-11 w-9 shrink-0 items-center justify-center rounded-full text-[#8b9cac] transition-colors hover:bg-[#eff7fc] active:bg-[#e2eff8] focus-visible:outline-2 focus-visible:outline-[#a9cbe1]"><MoreHorizontal aria-hidden="true" size={18} strokeWidth={1.7}/></button>
    ...
    </div>{managing === folder.id && <FolderInlineActions folder={folder} books={books} onAction={onFolderAction} onAddDay={() => { setPanel(null); onAdd("day", undefined, next); }} onClose={() => setManaging(null)}/>} {isOpen && <div className="ml-3 border-l border-[#e7edf2] pl-[19px]">{tree(folder.folders, next)}{folder.days.map(day => <button key={day.id} onClick={() => navigate(next, day.id)} className={`block min-h-11 w-full rounded-lg pl-2 pr-2 text-left text-sm text-[#4b5058] hover:bg-[#f7fafc] ${day.id === dayId ? "bg-[#eff7fc]" : ""}`}>{day.title}<span className="ml-2 text-xs text-[#939ba5]">{day.words.length}개</span></button>)}</div>}</div>;
  });

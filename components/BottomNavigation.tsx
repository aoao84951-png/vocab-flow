...
export default function BottomNavigation({ books, step, path, dayId, onHome, onNavigate, onAdd }: Props) {
  ...
  return <>
    {panel && <div className="fixed inset-0 z-[70] bg-black/15" onClick={() => setPanel(null)}>
      <section role="dialog" aria-modal="true" aria-label={{search:"전체 검색",stars:"중요 단어",add:"추가",contents:"목차"}[panel]} onClick={e => e.stopPropagation()} className="absolute inset-x-0 bottom-[calc(76px+env(safe-area-inset-bottom))] mx-auto flex max-h-[72dvh] max-w-[640px] flex-col rounded-t-3xl bg-white px-5 pt-5 pb-4 shadow-xl sm:bottom-[108px] sm:rounded-3xl">

...
export default function BottomNavigation({ books, step, path, dayId, onHome, onNavigate, onAdd, onFolderAction }: Props) {
  ...
  const drag = useFolderDrag(panelScrollRef, panel === "contents", onFolderAction);
  const scrollHideTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  useEffect(() => {
    panelScrollRef.current?.removeAttribute("data-scrolling");
    return () => {
      if (scrollHideTimer.current !== null) clearTimeout(scrollHideTimer.current);
    };
  }, [panel, step]);
  const showPanelScrollbar = () => {
    panelScrollRef.current?.setAttribute("data-scrolling", "true");
    if (scrollHideTimer.current !== null) clearTimeout(scrollHideTimer.current);
  ...
  const [contentsLocation, setContentsLocation] = useState(path);
  const [addTargetPath, setAddTargetPath] = useState<string[] | null>(null);
  ...
  const browsePath = panel === "contents" ? contentsLocation : panel === "add" && addTargetPath !== null ? addTargetPath : path;
  const addingDayId = panel === "contents" || panel === "add" && addTargetPath !== null ? "" : dayId;
  let current: Folder | undefined; let folders = books;
  for (const id of browsePath) { current = folders.find(folder => folder.id === id); folders = current?.folders ?? []; }
  ...
  const open = (value: typeof panel) => { setPanel(panel === value ? null : value); setQuery(""); setPickingDay(false); };
  const navigate = (target: string[], day?: string, index?: number) => { setPanel(null); onNavigate(target, day, index); };
  const add = (kind: "folder" | "day" | "word", day?: string) => { setPanel(null); onAdd(kind, day, panel === "add" && addTargetPath !== null ? addTargetPath : undefined); };
  const addWord = () => {
    if (addingDayId) add("word", addingDayId);
    else if (current?.days.length === 1) add("word", current.days[0].id);
    else if (current?.days.length) setPickingDay(true);
    else add("day");
  };
  const addOptions = [
    ...(current ? [{ label: current.days.length || addingDayId ? "단어 추가" : "Day 먼저 만들기", run: addWord }] : []),
    ...(!addingDayId ? [
  ...
  const openAdd = () => {
    if (panel === "add") { setPanel(null); return; }
    if (panel === "contents") { setAddTargetPath(contentsLocation); setPickingDay(false); setPanel("add"); return; }
    setAddTargetPath(null);
    if (addOptions.length === 1) { setPickingDay(false); addOptions[0].run(); }
    else open("add");
  };
  if (!visible) return null;
  return <>
    {drag && <div aria-live="polite" className="pointer-events-none fixed z-[100] max-w-64 truncate rounded-xl bg-[#527895] px-4 py-2 text-sm text-white shadow-lg" style={{ left: Math.max(8, Math.min(drag.x + 12, window.innerWidth - 220)), top: Math.max(8, drag.y - 56) }}>{drag.title} · {drag.drop ? drag.drop.position === "inside" ? "안으로 이동" : "이 위치로 이동" : "놓을 위치를 선택하세요"}</div>}
    {panel && <div className="fixed inset-0 z-[70] bg-black/15" onClick={() => setPanel(null)}>
      <section role="dialog" aria-modal="true" aria-label={{search:"전체 검색",stars:"중요 단어",add:"추가",contents:"목차"}[panel]} onClick={e => e.stopPropagation()} className="absolute inset-x-3 bottom-[calc(98px+var(--mobile-bottom-inset))] mx-auto flex max-h-[calc(100dvh-140px-var(--mobile-bottom-inset)-env(safe-area-inset-top))] max-w-[640px] flex-col rounded-3xl bg-white px-5 pt-5 pb-4 shadow-xl sm:inset-x-0 sm:bottom-[108px] sm:max-h-[72dvh]">
        <div className={`${panel === "contents" ? "mb-1" : "mb-4"} flex shrink-0 items-center justify-between`}><h2 className={`min-w-0 flex-1 break-words ${panel === "contents" ? "text-[15px] font-medium" : "text-lg font-bold"}`}>{{search:"전체 검색",stars:"중요 단어",add:`${current?.days.find(day => day.id === addingDayId)?.title || current?.title || "단어장"}에 추가`,contents:"목차"}[panel]}</h2><div className="flex shrink-0 items-center gap-2"><button autoFocus aria-label="닫기" onClick={() => setPanel(null)} className="flex h-10 w-10 items-center justify-center"><X size={19}/></button></div></div>
        <div ref={panelScrollRef} onScroll={showPanelScrollbar} data-bottom-panel-scroll className="min-h-0 overflow-y-auto overscroll-contain">
          ...
          {panel === "contents" && <BookContents books={books} initialPath={path} selectedDayId={dayId} onNavigate={navigate} onFolderAction={onFolderAction} drag={drag} onPageChange={() => panelScrollRef.current?.scrollTo({ top: 0 })} onLocationChange={setContentsLocation} onAdd={(kind, targetPath) => { setPanel(null); onAdd(kind, undefined, targetPath); }} />}

...
export default function SelectionToolbar(){
 ...
 return createPortal(<div ref={toolbar} role="toolbar" aria-label="텍스트 서식" style={{visibility:"hidden",fontFamily}}
  ...
  {palette&&<div className="max-h-[300px] overflow-y-auto overscroll-contain border-t border-[#edf0f4] p-2">{[['글자색',paletteColors,'foreColor'],['배경색',[...backgrounds,...custom],'hiliteColor']].map(([label,values,action])=><div key={String(label)}><p className="mb-2 mt-2 text-xs font-semibold text-[#858b94]">{String(label)}</p><div className="grid grid-cols-5 gap-2">{(values as string[]).map((color,index)=><button type="button" key={color} aria-label={`${label} ${index===0?'기본':color}`} title={color} className="h-8 rounded-lg border border-[#dce2e9] text-lg font-semibold" style={{color:action==='foreColor'?color:'#535b65',backgroundColor:action==='hiliteColor'?color:undefined}} onClick={()=>{command(String(action),color);}}>{action==='foreColor'?'A':index===0?'∅':''}</button>)}</div></div>)}<div className="mt-3 border-t border-[#edf0f4] pt-2"><div className="flex gap-1 rounded-lg bg-[#f7f5fb] p-1"><button className={`flex-1 rounded-md py-1 text-xs ${paletteTab==='text'?'bg-white':''}`} onClick={()=>setPaletteTab('text')}>글자색</button><button className={`flex-1 rounded-md py-1 text-xs ${paletteTab==='background'?'bg-white':''}`} onClick={()=>setPaletteTab('background')}>배경색</button></div><div className="mt-2 flex gap-1"><input aria-label="내 색상 코드" value={customHex} onChange={e=>setCustomHex(e.target.value)} className="min-w-0 flex-1 rounded-lg border border-[#dce2e9] px-2 text-xs"/><button aria-label="내 색상 추가" className="rounded-lg bg-[#f0eafc] px-2" onClick={()=>{if(!/^#[\da-f]{6}$/i.test(customHex))return;const next=Array.from(new Set([...custom,customHex.toLowerCase()]));setCustom(next);localStorage.setItem('vocab-custom-colors',JSON.stringify(next));command(paletteTab==='text'?'foreColor':'hiliteColor',customHex);}}><Plus size={14}/></button></div></div></div>}
 </div>,document.body);
}

...
export default function DailyHome({books,onOpen,onContents}:{books:Folder[];onOpen:(path:string[],dayId:string,index:number)=>void;onContents:()=>void}){
 ...
 return <div className="mx-auto w-full max-w-[860px] px-5 pt-7 pb-8 sm:px-8 sm:pt-10">
  ...
  {resume&&<button onClick={()=>onOpen(resume.path,resume.dayId,resume.index)} className="mt-8 flex w-full items-center justify-between gap-4 rounded-2xl border border-[#e7edf2] p-5 text-left"><span className="min-w-0"><span className="block text-xs text-[#8a94a6]">이어서 보기</span><span className="mt-1 block truncate font-semibold text-[#4b5058]"><RichText text={resume.word.word}/></span><span className="mt-2 block"><PathChips parts={resume.location} /></span></span><ArrowUpRight size={18} className="shrink-0 text-[#7892a8]"/></button>}
 </div>;
}

function PathChips({parts,centered=false}:{parts:string[];centered?:boolean}) {
 return <span aria-label={`위치: ${parts.join(" → ")}`} className={`flex flex-wrap items-center gap-x-2 gap-y-2 ${centered ? "justify-center" : "justify-start"}`}>
  {parts.map((part,index)=><span key={index} className="inline-flex min-w-0 max-w-full items-center gap-2">
   {index>0&&<ChevronRight aria-hidden="true" size={10} strokeWidth={1.5} className="shrink-0 text-[#b5c0ca]"/>}
   <span className={`min-w-0 break-words rounded-lg px-2.5 py-1 text-[11px] leading-relaxed ${index===parts.length-1 ? "bg-[#e2f0fa] font-medium text-[#587c98]" : "bg-[#f0f3f6] text-[#7e8b98]"}`}>{part}</span>
  </span>)}
 </span>;
}

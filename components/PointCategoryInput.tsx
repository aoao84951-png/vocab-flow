...
export default function PointCategoryInput({ value, options, onChange }: { value: string; options: string[]; onChange: (value: string) => void }) {
  ...
  return <div className="relative w-[120px] shrink-0 sm:w-40">
    {custom && <RichTextField autoFocus aria-label="포인트 분류 직접 입력" value={value} onChange={event => onChange(event.target.value)} placeholder="직접 입력" className="h-11 w-full rounded-xl border border-[#ddeaf3] bg-white px-3 pr-8 text-[13px] outline-none focus:border-[#91bad6]" />}

...
export default function useFolderDrag(container: RefObject<HTMLDivElement | null>, enabled: boolean, onMove: (action: FolderAction) => void) {
  ...
  useEffect(() => {
    ...
    let pending: Omit<Drag, "drop"> | null = null;
    ...
    const locate = (x: number, y: number): Drop | null => {
      ...
      if (isDay || path.includes(active.id)) return null;
      const rect = row.getBoundingClientRect();
    ...
    const finish = () => {
      if (active?.drop) {
        ...
        if (active.itemType === "day") callback.current({ kind: "move-day", id: active.parentId, dayId: id, relativeTo: drop.id, placement: drop.position === "before" ? "before" : "after" });
        else callback.current({ kind: "move", id, destination: drop.destination, ...(drop.position !== "inside" ? { relativeTo: drop.id, placement: drop.position } : {}) });
      }
      clear();
      // A moved row may miss the release click; never swallow the next deliberate click.
